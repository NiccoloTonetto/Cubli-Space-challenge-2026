...
This script doubles as the command console: type the commands
CubliBalance.ino's handleCommandLine() understands (a1, a0, g<0..1>,
o<deg>, e) into the terminal you launched this script from and press
Enter -- they're sent as UDP packets to the XIAO.
Link-level commands:
    l0 / l1   -- switch the Teensy's link mode (USB / WiFi) live, no reboot
    x0 / x1   -- switch the XIAO's own mode (WIFI_TEST / TEENSY_BRIDGE)
    m0 / m1   -- switch the cube's control law (EDGE / CORNER). This plot
                 window only parses the 10-field edge format, so m1 will
                 make the stream go quiet here -- that is the corner build
                 talking, not a fault. Use telemetry_python_wifi_corner.py
                 or the dashboard for that mode.

NOTE: 'l', not 't'. The standalone EdgeBalance_WiFi.ino used t<0/1> for link
mode; the fused CubliBalance.ino kept the corner build's 'l' and dropped 't'.

A background thread also sends a lightweight "k" heartbeat every
HEARTBEAT_INTERVAL_S seconds -- CubliBalance.ino auto-disarms if it doesn't
hear anything on its WiFi link for kLinkTimeoutMs (3 s in the firmware), so
this keeps idle periods between real commands from tripping that failsafe.
It also keeps the XIAO's learned "laptop address" fresh, since
xiao_teensy_bridge.ino only knows where to send telemetry once it's heard
from this script at least once. See heartbeat_thread() for why 'k' and NOT
'h' -- this matters, 'h' is HALT on the current firmware.

Requires: pip install matplotlib   (no pyserial needed -- plain UDP socket)

Usage:
    1. The Teensy must be in EDGE mode (send m0). That is the mode whose
       telemetry is the 10-field comma CSV this script parses.
...
def heartbeat_thread(sock, xiao_addr, stop_event):
    ...
    SENDS 'k', NOT 'h'. This used to send "h", which the standalone
    EdgeBalance_WiFi.ino accepted as a keepalive specifically so this script
    would work unmodified. CubliBalance.ino inherited the corner grammar, where
    'h' is HALT -- a bare "h" there parses as atof("") = 0.0 -> h0 -> RESUME,
    ten times a second, which holds the HALT button down in the released
    position. 'k' is a no-op keepalive on every build in the tree.
    """
    while not stop_event.is_set():
        sock.sendto(b"k\n", xiao_addr)
