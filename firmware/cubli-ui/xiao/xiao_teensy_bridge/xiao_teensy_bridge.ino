/*
  ...
  ---------------------------------------------------------------------
  Companion to ../../teensy/CubliBalance/CubliBalance.ino. Relays that
  sketch's Serial1 telemetry/command protocol over WiFi/UDP to the laptop
  dashboard (../../dashboard/app.py), so the cube can balance untethered
  from USB.
  ...
  'x' packets are handled locally and NEVER relayed to the Teensy. This is
  load-bearing on the Teensy side too: CubliBalance.ino deliberately binds
  nothing to 'x', because a command on that letter could never arrive.

  Every other packet -- a/m/g/c/e/o/h/d/l/k/s, CubliBalance.ino's whole
  command grammar -- is relayed to Serial1 verbatim, and every line the
  Teensy sends back on Serial1 is forwarded to the laptop as one UDP
  packet, regardless of which mode is active, so the bridge itself is
  always live. THE BRIDGE IS DELIBERATELY DUMB: it does not parse, filter
  or whitelist Teensy commands, so adding a command letter to the firmware
  needs no change here.

  loop() is fully non-blocking (no delay() in the hot path) so the UDP
  command check runs every single iteration and commands are never missed,
  no matter what else the board is doing. That matters most for a0.

  Wiring to the Teensy (see CubliBalance.ino's header, and ../../docs/WIRING.md):
...
bool connectWiFi() {
  ...
  //   - the PC keepalive arrives in bursts, so the Teensy's link watchdog
  //     can time out and auto-disarm mid-balance for no real reason -- a
  //     phantom fault that looks exactly like a link failure. (The timeout
  //     is 3 s now, not the 300 ms this was first measured against, so the
  //     margin is far better -- but the burstiness is still real and the
  //     radio still must not sleep.);
...
void pollUdpIn() {
  ...
  // Relay everything else to the Teensy verbatim (a/m/g/c/e/o/h/d/l/k/s),
  // regardless of current mode -- the bridge itself is always live. No
  // filtering on purpose: see the header.
