// ############################################################################
// STANDALONE EDGE BUILD -- A/B REFERENCE FOR CubliBalance.ino
// ############################################################################
// Verbatim copy of
//   firmware/3D model/Gam/PreFINAL/WIFIMODE/EdgeBalance_WiFi/
// kept here as an INDEPENDENT FLASH TARGET so the fused sketch can be checked
// against the build it was derived from. Edge only: one wheel, no corner law,
// no m0/m1. Not one line of the control path differs from ../CubliBalance/'s
// edge mode -- that is the point of having it.
//
// AGAINST THE DASHBOARD, flashed with this:
//   works    arm/disarm (0.5 deg server-side gate; the FIRMWARE gate here is
//            9.58 deg -- see kArmGate below), gain, re-resolve 'e', trim
//            'o<deg>', 'k' keepalive, 10-column edge telemetry, charts,
//            3D view, recording
//   SILENT   *** the HALT / RESUME buttons do NOTHING. *** They send h1/h0,
//            and on THIS build 'h' is a KEEPALIVE, not halt -- h1 is swallowed
//            as a no-op. The fused sketch binds 'h' to HALT, which is why the
//            dashboard offers the button. There is no halt command at all in
//            this build. Use a0.
//   silent   the EDGE / CORNER mode buttons (no 'm' here), and the link-mode
//            command: the dashboard sends l<0/1>, this build wants t<0/1>.
//   absent   no "# state=" line, so the dashboard infers the mode from the
//            10-field packet width. It will read EDGE.
//
// TWO DIFFERENCES FROM THE FUSION THAT ARE NOT BUGS, both deliberate:
//   - IMU ODR is 400 Hz here, 800 Hz in ../CubliBalance/. The 500 Hz control
//     loop polls faster than 400 Hz produces, so ~1 read in 5 is a stale
//     repeat. The fusion took the corner build's 800 Hz fix.
//   - 'h' is the keepalive here (see above).
// Everything else -- gains, trips, saturation, the candidate table, kArmGate's
// suspect 9.58 deg -- is identical in both.
//
// Flash ../CubliBalance/ to get halt and the mode switch.
// ############################################################################
