...
// CORNER STAGE 2: RATE (om) ONLY  [WiFi build]
// ============================================================================
// WiFi variant of ../../../../corner-bringup/Stage2_RateOnly/. The control
// path -- corner table, estimator, state masking, gains, trips -- is copied
...
//
//   a<0/1>    arm / disarm            c         re-resolve corner
//   h<0/1>    halt
//
...
//
// ------------------------------- WATCHDOG -----------------------------------
// From this stage onward there is a gArmed to drop, so the link watchdog
// DISARMS if no line arrives on Serial1 for kLinkTimeoutMs. Stage 1 had no
// arm and cancelled its pulse instead. terminal_wifi.py sends "k" every
// 100 ms, well inside the 300 ms budget.
//
// ------------------------------- WIRING -------------------------------------
...
// 3x9 dot product. Masking the STATE, not the gain, is what keeps all five
// corner stages sharing byte-identical Kp data.
//
// No position feedback yet, so there is no term that can reinforce a sign
// error into a runaway -- cheapest closed-loop confidence available.
//
// This file also carries the full safety scaffold (latching-by-cycle disarm
// on tilt/rate/NaN). There is a SINGLE gArmed for the whole corner
// controller -- all three wheels arm and disarm together.
//
...
//       wrong somewhere. Go back to Stage 1, do not proceed to Stage 3.
// ============================================================================
...
static const float kMaxTilt    = 0.24f;    // rad, ~14 deg -- wide on purpose this
                                             // stage (needs to clear real stiction at
                                             // low authority). Compared against
                                             // norm3(phi). NOT the Stage 4/5 value.
...
static const float kTaperStart = 36.0f;    // rad/s, 90% of cap -- smoother than a
                                             // hard cutoff, already hardware-validated

// Carried forward from edge-bringup, all three CONFIRMED +1.0f, re-confirmed
// in this folder's own Stage 1.
...
// ----------------------------------------------------------------------------
// The USB build's a/c/h grammar, UNCHANGED -- the link controls live on k/l/d
// instead. `echo` is whichever stream the line arrived on, so an ack goes back
// down the channel it came in on.
...
void handleCommandLine(const char* line, Stream& echo) {
  ...
  if (cmd == 'a') {
    ...
    echo.println("# unknown. use: a<0/1>  c (re-resolve corner)  h<0/1>  "
...
void setup() {
  ...
  gBoot.println("\nstarted - CORNER STAGE 2: RATE ONLY (WiFi build, cube held by hand)");
  ...
  gBoot.println("# STARTS DISARMED. Send a1 to arm, a0 to disarm.");
...
// NOTES
// ----------------------------------------------------------------------------
// Next: ../Stage3_PositionDamping_WiFi/ -- adds the phi block, with a
// hand-ramped gGainScale starting at 0.1. Do not proceed if any direction
// felt like it was helping the fall rather than resisting it.
// ============================================================================
