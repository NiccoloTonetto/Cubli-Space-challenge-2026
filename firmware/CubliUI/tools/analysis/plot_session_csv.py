...
# Reference line only. cubli_gains.h's ARM_GATE, which is what the number
# SHOULD be. CubliBalance.ino gates only EDGE mode, and at a suspected-typo
# 9.58 deg; CORNER mode has no gate at all. So read this line as "the intended
# equilibrium tolerance", not as "what the firmware enforced during this run".
ARM_GATE_DEG = 0.5
...
# The recording scripts all write into CubliUI/telemetry/, split by the mode
# that produced the file: telemetry/plot/ for the live plotters' and the
# dashboard's PLOTMODE csv, telemetry/serial/ for terminal_wifi.py's
# SERIALMONITORMODE logs. The search starts at CubliUI/ rather than at
# telemetry/, so a capture saved somewhere else -- an older run, a file dropped
# in by hand, a USB-side recording -- is still reachable instead of invisible.
#
# parents[2] == CubliUI/ (plot_session_csv.py -> analysis -> tools -> CubliUI).
