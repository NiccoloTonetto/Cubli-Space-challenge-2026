...
// CORNER STAGE 3: POSITION (phi) + RATE (om), RHO STILL ZERO  [WiFi build]
// ============================================================================
// WiFi variant of ../../../../corner-bringup/Stage3_PositionDamping/. The
// control path -- corner table, estimator, state masking, gains, trips -- is
// copied VERBATIM and must not be retuned here. Retune in the USB sketch and
// re-copy. Everything this file adds is transport; see
// ../Stage1_CornerIDAndPulse_WiFi/ for the full rationale of the link layer,
// which is identical in all four corner bring-up WiFi stages.
//
// ---------------------------- COMMAND GRAMMAR -------------------------------
// This stage's own commands are UNCHANGED from the USB build:
//
//   a<0/1>    arm / disarm            g<0..1>   gain scale
//   m<deg>    set kMaxTilt            c         re-resolve corner
//   h<0/1>    halt
//
...
//
// The link watchdog DISARMS if no line arrives on Serial1 for kLinkTimeoutMs.
// terminal_wifi.py sends "k" every 100 ms, well inside the 300 ms budget.
//
// >>> This is the first stage where a sign error can reinforce into a runaway
// AND the operator is on the far end of a radio link. Ramp gGainScale by hand
// exactly as on USB -- do not shortcut to g1.0 because the cube is now
// untethered and convenient to hold. <<<
//
// ------------------------------- WIRING -------------------------------------
...
// management) stays masked to zero until Stage 4. gGainScale starts low and
// is ramped up manually between runs rather than jumping to full authority.
//
...
// Same semantics as every panel/edge/corner stage from Stage 2 onward:
...
// ----------------------------------------------------------------------------
// Tab-delimited, 18 fields, byte-for-byte the USB build's format -- the whole
...
//   rho_x rho_y rho_z  tau_x tau_y tau_z  tau_cmd_x tau_cmd_y tau_cmd_z
//   armed gain_scale
...
void printState(Stream& out, uint32_t t_ms, const float rho[3], const float tau[3],
...
// NOT const, live-settable with "m<deg>": wide enough that low-gGainScale
// steps can reach a commanded torque above real stiction before tripping.
// Compared against norm3(phi). NOT the Stage 4/5 value.
...
// ----------------------------------------------------------------------------
// The USB build's a/g/m/c/h grammar, UNCHANGED -- the link controls live on
...
void handleCommandLine(const char* line, Stream& echo) {
  ...
  if (cmd == 'a') {
    ...
    echo.print("# kMaxTilt = "); echo.print(val, 2); echo.println(" deg");
  } else if (cmd == 'c') {
    ...
    echo.println("# unknown. use: a<0/1>  g<0..1>  m<deg>  c (re-resolve)  h<0/1>  "
...
void setup() {
  ...
  gBoot.println("\nstarted - CORNER STAGE 3: POSITION + RATE (WiFi build, cube held by hand)");
  ...
  gBoot.println("# STARTS DISARMED, gGainScale=0.1. Send a1 to arm, a0 to");
  gBoot.println("# disarm, g<0..1> to step gain, m<deg> to widen/narrow kMaxTilt.");
  ...
  gBoot.println("#   on Serial1 for 300 ms.");
...
void loop() {
  ...
  // mode, so losing it must disarm rather than leave the law running blind.
  ...
  if (++gTelemetryCount >= gTelemetryDecim) {
    gTelemetryCount = 0;
    printState(activeStream(), time, rho, gLastTau, gLastTauCmd, gArmed, gGainScale);
  }
}   // end of loop()
...
// NOTES
// ----------------------------------------------------------------------------
// Next: Stage 4 is ../../CornerBalance_WiFi/ -- the full 9-state law with the
// rho block live plus friction feedforward. It is NOT duplicated in this
// folder; it is the validated build, and it can be read either with
// ../../terminal_wifi.py (set its TELEMETRY_MODE to SERIALMONITORMODE) or
// plotted with ../../telemetry_python_wifi_corner.py (PLOTMODE, the default).
//
// After that, ../Stage5_Release_WiFi/ tightens the trip policy for release.
//
// Do not proceed if any direction fought the wrong way at g=1.0.
// ============================================================================
