#!/usr/bin/env python3
"""build_web.py -- gzip index.html into a PROGMEM byte array for the XIAO.

    python build_web.py            # -> ../xiao_cubli_console/web_index.h
    python build_web.py --check    # exit 1 if the header is stale (CI / pre-flash)
...
SRC = HERE / "index.html"
DST = HERE.parent / "xiao_cubli_console" / "web_index.h"
...
def main() -> int:
    ...
    if args.check:
        if not DST.exists():
            print(f"STALE: {DST.name} does not exist. Run: python build_web.py",
                  file=sys.stderr)
            return 1
        if DST.read_text(encoding="utf-8") != out:
            print(f"STALE: {DST.name} does not match index.html. "
                  f"Run: python build_web.py", file=sys.stderr)
            return 1
