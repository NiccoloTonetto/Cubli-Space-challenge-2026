...
// and arm gate as ../Stage4_FullLaw/Stage4_FullLaw.ino (which is itself a
// byte-identical copy of edge-bringup/Stage4_FullLaw/Stage4_FullLaw.ino, the
// hardware-validated build). NOTHING in the control path is changed here.
//
// What this file adds, and only this:
//   - a second telemetry/command channel on Serial1, wired to a XIAO ESP32C6
//     running firmware/XIAO/xiao_teensy_bridge/xiao_teensy_bridge.ino, so the
//     cube can balance on its edge untethered from USB;
...
// ----------------------------------------------------------------------------
// Companion files:
//   ../Stage4_FullLaw/Stage4_FullLaw.ino  -- the USB-tethered reference build
//     this is derived from, kept byte-identical to the hardware-validated
//     edge-bringup copy. Retune THERE, then re-copy the changed constants.
//   firmware/XIAO/xiao_teensy_bridge/xiao_teensy_bridge.ino  -- the XIAO
//     ESP32C6 sketch that relays Serial1 <-> UDP, reused as-is (no edit
//     needed beyond WiFi credentials / static IP).
//   matlab/2Dmodel/Validation/telemetry_python_wifi.py  -- the PC-side
//     viewer/logger/command console. Works against this sketch unmodified:
//     same 10-field PLOTMODE CSV, same commands, and its "h" keepalive is
//     accepted here. Its plot labels say theta/theta_dot -- read those as
//     phi_edge/om_edge.
//
// See README.md in this folder for the end-to-end flashing procedure.
// ============================================================================
