...
// CORNER STAGE 4: FULL LAW + AUTOMATIC TRIM  [WiFi build]
// ============================================================================
// WiFi variant of ../../../../corner-bringup/Stage4_AutoTrim/. The control
// path -- corner table, estimator, full law, friction feedforward, arm gate,
// the loosened velocity cap, AND the whole automatic-trim mechanism (gTrim,
// updateTrim(), applyTrimGuards(), the VERIFIED adaptation sign) -- is copied
// VERBATIM and must not be retuned here. Retune in the USB sketch and
// re-copy. Everything this file adds is transport; see
...
// derivation of the trim law, the guard table, and the ">>> THE SIGN IS
// VERIFIED, DO NOT RE-DERIVE <<<" warning. None of that is repeated here,
...
//   it in over the next ~4 tau_a (~4 min at the default 60 s).
...
//
// Watching trim converge takes minutes, and 250 Hz is unreadable for that.
// "d20" (25 Hz) is the useful rate for this stage; "d2" puts it back for
// capture. Changing it re-prints the header, so the log stays self-describing.
// ============================================================================
...
// This stage's line is 26 fields (~205 B), the widest in the folder: ~52 kB/s
// at 250 Hz, comfortable against the 1 Mbaud link, but the UDP hop is the
// real ceiling (~105 lines/s measured in link-bringup Stage 6).
...
//
// The last five are this stage's whole point: per-axis trim in degrees, the
// equivalent COM offset in mm (norm3(gTrim) * ell, small-angle) and whether
// adaptation is currently running. 26 columns is outside
// ../../plot_session_csv.py's 10/21 auto-detection, same as Stages 1-3 --
// these logs are records to read or post-process.
...
void printState(Stream& out, uint32_t t_ms, const float rho[3], const float rhoLp[3],
  ...
  out.print('\t'); out.println(gTrimEnabled ? 1 : 0);
}
...
// with "n<value>" (the USB build's "k<value>" -- see the header for why it
// had to move) if 60 s turns out too slow/fast to watch converge -- the
// note's own table: 20s->8.765e-6  30s->5.843e-6  60s->2.922e-6
...
// ----------------------------------------------------------------------------
// The USB build's a/g/c/z/h grammar, plus the two letters the transport
...
void handleCommandLine(const char* line, Stream& echo) {
  ...
  if (cmd == 'a') {
    ...
    echo.println("# unknown. use: a<0/1>  g<0..1>  c (re-resolve)  "
                  ...
                  "n<value> (gKAdapt)  h<0/1>  d<N> (telem decim)  "
                  "l<0/1> (link)  k (keepalive)");
  }
}
...
void setup() {
  ...
  gBoot.println("\nstarted - CORNER STAGE 4: FULL LAW + AUTOMATIC TRIM (WiFi build, cube held by hand)");
  ...
  gBoot.println("#   d20 = 25 Hz readable -- use it, convergence takes minutes),");
...
void loop() {
  ...
  // throttled, and the trim adaptation is NOT -- it integrates every cycle
  // regardless of what the decimation is set to.
...
// NOTES
// ----------------------------------------------------------------------------
// This file vs ../Stage4_FullLaw_WiFi/: identical control law, identical
// corner candidate table, identical velocity-cap/friction-FF/arm-gate policy
// -- the ONLY behavioral difference is that gTrim (automatic, adapting) has
// replaced gPhiOffset (manual, snapshot-and-freeze), and the telemetry line
// grew from 21 to 26 fields. If something looks wrong here that isn't about
// trim specifically, check whether the FullLaw build has the same problem
// before assuming this file's changes caused it.
//
// This file vs ../../../../corner-bringup/Stage4_AutoTrim/: transport only.
// The link layer (LineReader, DualPrint, watchdog, k/l/d) is the same one
// every corner bring-up WiFi stage uses, and the two remapped command letters
// are documented in the header. Retune in the USB sketch, then re-copy.
//
// Next steps once trim behavior is trusted on real hardware (unchanged from
// the USB build's list):
//   - Port the SAME replacement into Stage5_Release (both builds).
