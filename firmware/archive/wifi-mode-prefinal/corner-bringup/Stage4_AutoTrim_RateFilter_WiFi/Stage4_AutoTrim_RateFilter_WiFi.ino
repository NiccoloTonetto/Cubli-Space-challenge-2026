...
// CORNER STAGE 4: FULL LAW + AUTOMATIC TRIM + RATE LOW-PASS  [WiFi build]
// ============================================================================
// WiFi variant of ../../../../corner-bringup/Stage4_AutoTrim_RateFilter/. The
// control path -- corner table, estimator, full law, friction feedforward, arm
// gate, the loosened velocity cap, the whole automatic-trim mechanism (gTrim,
// updateTrim(), applyTrimGuards(), the VERIFIED adaptation sign), AND this
// stage's two new pieces (the 20 Hz rate low-pass w_filt, and gKAdapt's raised
// 1e-4 default) -- is copied VERBATIM and must not be retuned here. Retune in
// the USB sketch and re-copy. Everything this file adds is transport; see
// ../Stage1_CornerIDAndPulse_WiFi/ for the full rationale of the link layer,
// which is identical in all the corner bring-up WiFi stages.
//
// Read the USB sketch's header FIRST if you have not: it carries the entire
// derivation of the trim law, the guard table, the ">>> THE SIGN IS VERIFIED,
// DO NOT RE-DERIVE <<<" warning, and hw-run-analysis.md's 35 Hz finding that
// motivates the rate filter. None of that is repeated here, and none of it
// changed.
//
// ------------------------ WHAT THIS STAGE ADDS, IN BRIEF --------------------
// Two changes vs ../Stage4_AutoTrim_WiFi/, both from hw-run-analysis.md's
// study of a 373.5 s continuous corner balance ("corner-balance-373s-2026-08-20.log"):
//
//   1. FIX 4.1 -- a first-order low-pass (default 20 Hz) on the body rate,
//      feeding ONLY the control law's om block. That log found torque
//      saturated 71.8% of the time with the K2*om term alone demanding 1.5x
//      the available torque, driven by a ~35 Hz structural mode riding on the
//      gyro. A 1.3 Hz closed loop cannot damp a 35 Hz mode -- it can only feed
//      it. The filter keeps that energy out of the gain; at 20 Hz it costs
//      ~3.7 deg of phase at 1.3 Hz against a ~70 deg margin.
//   2. FIX 4.5 -- gKAdapt's default raised from 2.922e-6 (tau_a = 60 s) to
//      1e-4, ~21x below the stability limit re-derived on the 15-state
//      augmented model, converging in ~10 s instead of ~4 min.
//
// w_filt is a SEPARATE signal, not a replacement for w_b. Raw w_b still drives
// ghat propagation, telemetry's om_x/y/z_dps columns and every safety check
// (the isfinite() trips) unfiltered. The one deliberate extra use of w_filt is
// updateTrim()'s quiescence gate -- see the USB header for that justification.
//
// ---------------------------- COMMAND GRAMMAR -------------------------------
// This stage's own commands, with TWO letters remapped -- see below:
//
//   a<0/1>    arm / disarm (gated on the 0.5 deg arm gate)
//   g<0..1>   gain scale
//   c         re-resolve corner
//   z<0/1>    z1 fast-start seeds gTrim so corrected phi reads ~0, z0 clears
//   y<0/1>    freeze / resume trim adaptation   [USB build: 'x']
//   n<value>  set gKAdapt, the adaptation gain  [USB build: 'k']
//   f<Hz>     rate low-pass corner frequency, sweep 15-25 Hz live
//   h<0/1>    halt
//
...
//
// 'f' survives the link unchanged -- it is free in every other corner stage
// and the bridge relays it verbatim. Only the two letters below had to move,
// and they moved for the same reasons they moved in ../Stage4_AutoTrim_WiFi/.
//
// ---------------------- WHY TWO LETTERS HAD TO MOVE -------------------------
// Every other corner bring-up WiFi stage keeps its USB letters byte-identical.
// This one cannot, and both exceptions are forced by the transport, not by
// taste:
//
//   1. 'x' NEVER REACHES THIS BOARD. 'x' packets are consumed by the XIAO
//      bridge itself (x0 = WIFI_TEST, x1 = TEENSY_BRIDGE) and are never
//      relayed -- see xiao_teensy_bridge.ino's pollUdpIn(). Leaving the trim
//      freeze on 'x' would make it work over USB and be silently unreachable
//      over WiFi. It is 'y' here (same polarity: y1 = adapting, y0 = frozen).
//
//   2. 'k' IS THE LINK KEEPALIVE. terminal_wifi.py sends a bare "k" every
//      100 ms. If gKAdapt stayed on 'k', every keepalive would parse as
//      atof("") = 0.0 and set the adaptation gain to ZERO ten times a second
//      -- trim would sit frozen at 0 and look, from the telemetry, exactly
//      like a converged trim of zero. That is a silent-wrong-answer failure,
//      which is worse than a loud one. gKAdapt is 'n' here.
//
// Nothing else moved. 'h' is still halt (unlike ../CornerBalance_WiFi/, which
// predates this folder and binds h = keepalive, p = halt -- see
// ../README.md's "one legacy difference").
//
// ------------------------ LINK LOSS FREEZES THE TRIM ------------------------
// The watchdog's auto-disarm also stops the trim adapting, for free and with
// no extra state: updateTrim() is gated on gArmed, exactly as it is gated for
// a trip in the USB build. "Do not learn from a fall" extends to "do not
// learn from a dropped radio link" without a line of code. The converged
// value is KEPT, not cleared -- re-arming resumes from where it was.
//
// The RATE FILTER is deliberately NOT frozen or reset by any of that. It is a
// measurement filter, not learned state: it keeps tracking w_b whether the
// board is armed, disarmed, halted or link-dead, so the moment you re-arm the
// om block sees a settled signal rather than a filter converging from zero.
//
// ------------------------------- WIRING -------------------------------------
...
//
// ============================================================================
// STAGE 4 CHECKLIST — cube held by hand, gGainScale = 1.0 (unchanged from the
// USB build):
...
//   start, or arm anyway once close enough and let automatic adaptation walk
//   it in over the next ~4 time constants (~10 s at this file's k_a = 1e-4).
//   [ ] trim_x/y/z_deg in telemetry moves smoothly toward a steady value
//       while armed and quiescent, then stops changing -- that's
//       convergence. Note the value: it is your equivalent COM-error readout
//       (also printed as trim_com_mm).
//   [ ] All three wheels unwind after each correction (watch the rho_lp
//       columns) once trim has converged -- persistent nonzero standing speed
//       after convergence means something is still wrong.
//   [ ] om_x/y/z_filt_dps sits visibly quieter than the raw om_x/y/z_dps
//       beside it. That gap IS the 35 Hz mode being taken out of the K2 path;
//       if the two columns track each other closely, either the mode is not
//       present in this run or the filter is not doing anything -- check
//       gRateFilterHz before concluding the fix worked.
//
// Watching trim converge is fast now (~10 s), but 250 Hz is still unreadable
// for it, and the raw-vs-filtered comparison above is much easier to eyeball
// slowly. "d20" (25 Hz) is the useful rate for this stage; "d2" puts it back
// for capture. Changing it re-prints the header, so the log stays
// self-describing.
// ============================================================================
...
// This stage's line is 29 fields (~230 B), the widest in the folder -- three
// more than ../Stage4_AutoTrim_WiFi/'s 26, for the filtered-rate columns:
// ~58 kB/s at 250 Hz, comfortable against the 1 Mbaud link, but the UDP hop is
// the real ceiling (~105 lines/s measured in link-bringup Stage 6).
...
// "f<Hz>" to sweep the range on the bench without reflashing -- and over
// WiFi 'f' needed no remapping, unlike this stage's y/n pair.
...
struct CornerCandidate {
  ...
  float ellM;       // m, contact-to-COM lever arm -- used only to convert a
                     // converged trim into an equivalent COM offset in mm for
                     // telemetry (Automatic Trim.md S4's "log trim
                     // continuously" guard).
};
...
// AUTOMATIC TRIM state -- see the USB sketch's header for the full
// derivation. ADDED to the raw measurement below (note the sign: this is
// deliberately the opposite convention from Stage4_FullLaw's gPhiOffset,
// which was SUBTRACTED -- gTrim follows "Automatic Trim...md" section 4's own
// phi_err = raw + trim verbatim, because the adaptation law's sign is the one
// thing in that note explicitly flagged as VERIFIED, DO NOT RE-DERIVE).
static float gTrim[3]     = { 0.0f, 0.0f, 0.0f };   // rad, added to raw phi
static bool  gTrimEnabled = true;                    // "y0" freezes, "y1" resumes
...
// ----------------------------------------------------------------------------
// Tab-delimited, 29 fields, byte-for-byte the USB build's format -- the whole
...
//
// The trim five are Stage 4-AT's whole point; the filtered-rate three are
// this stage's. om_x/y/z_dps stay RAW so the two can be compared column
// against column in one log -- that difference is the 35 Hz mode being kept
// out of the K2 path, and it is the only direct evidence the fix is working.
//
// 29 columns is a format ../../plot_session_csv.py already recognizes
// (CORNER_TRIM_FILT_COLS), alongside 21 / 26 / 33. It is still outside the
// 10/21 auto-detection the interactive picker uses.
...
void printState(Stream& out, uint32_t t_ms, const float rho[3], const float rhoLp[3],
  ...
  // offset in mm per Automatic Trim.md S4's "log trim continuously" guard:
  // this is a live COM-error readout, not just an internal correction term,
  // so it's worth watching converge.
  out.print('\t'); out.print(gTrim[0] * (float)RAD_TO_DEG, 4);
  out.print('\t'); out.print(gTrim[1] * (float)RAD_TO_DEG, 4);
  out.print('\t'); out.print(gTrim[2] * (float)RAD_TO_DEG, 4);
  out.print('\t'); out.print(norm3(gTrim) * kCorners[gCornerIdx].ellM * 1000.0f, 3);
  out.print('\t'); out.print(gTrimEnabled ? 1 : 0);
  ...
  out.print('\t'); out.print(w_filt[0] * (float)RAD_TO_DEG, 2);
  out.print('\t'); out.print(w_filt[1] * (float)RAD_TO_DEG, 2);
  out.print('\t'); out.println(w_filt[2] * (float)RAD_TO_DEG, 2);
}
...
// job, not this one). Compared against norm3(phi).
...
// speed" the derivation is built on; no separate filter is kept here,
// reusing the one this file already computes for telemetry.

// hw-run-analysis.md 4.5: stability limit k_a < 0.00214 (15-state
// augmented model, tau_filt=5s -- see the USB header note). 1e-4 sits at
// ~21x margin and converges in ~10s per that analysis, replacing
// Stage4_AutoTrim's tau_a=60s (2.922e-6) default. Live-settable with
// "n<value>" (the USB build's "k<value>" -- see the header for why it had
// to move) -- NON-MONOTONIC per the source measurement (1e-4 converges
// faster than 1e-3), so re-measure before assuming "higher = faster".
...
void updateTrim(float dt) {
  // Freezes on disarm/any trip for free: gArmed goes false the same cycle a
  // trip fires (see the checks above in commandWheels()) OR the same cycle
  // the WiFi watchdog fires (see loop()), and this is gated on it -- "do not
  // learn from a fall", and "do not learn from a dropped link", without any
  // extra state.
  if (!gTrimEnabled || !gArmed) { return; }
  if (norm3(phi) >= kTiltQuiet)  { return; }   // recovering, not quiescent
  // w_filt, not raw w_b -- see the USB header note. hw-run-analysis.md's own
  // data showed phi clean (genuinely quiescent) while raw om was dominated
  // by the 35 Hz mode; gating on the raw signal would block adaptation
...
// ----------------------------------------------------------------------------
// The USB build's a/g/c/z/f/h grammar, plus the two letters the transport
// forced to move ('x' -> 'y', 'k' -> 'n'; see the header). The link controls
// live on k/l/d. `echo` is whichever stream the line arrived on, so an ack
...
void handleCommandLine(const char* line, Stream& echo) {
  ...
  if (cmd == 'a') {
    ...
    if (val != 0.0f) {
      // Fast-start seed: -= (not +=) because gTrim is ADDED to raw phi here
      // (opposite convention from Stage4_FullLaw's gPhiOffset, which was
      // subtracted) -- this makes corrected phi read ~0 right now.
      gTrim[0] -= phi[0];
      gTrim[1] -= phi[1];
      gTrim[2] -= phi[2];
      applyTrimGuards();
      echo.print("# gTrim SEEDED to: ");
      echo.print(gTrim[0] * (float)RAD_TO_DEG, 3); echo.print(",");
      echo.print(gTrim[1] * (float)RAD_TO_DEG, 3); echo.print(",");
      echo.print(gTrim[2] * (float)RAD_TO_DEG, 3); echo.println(" deg");
      if (norm3(gTrim) > 0.1745329f) {   // ~10 deg > kTrimMax already, so
                                          // this only fires right at the
                                          // 2 deg clamp boundary -- still
                                          // worth flagging.
        echo.println("# NOTE: seed hit/near the 2 deg clamp -- that's");
        echo.println("#   mechanical (shim it), not something trim fixes.");
      }
    } else {
      gTrim[0] = gTrim[1] = gTrim[2] = 0.0f;
      echo.println("# gTrim cleared to 0,0,0");
    }
  } else if (cmd == 'y') {
    // USB build's "x<0/1>". Same polarity: y1 = adapting, y0 = frozen at the
    // current value, neither disarms. 'x' is eaten by the XIAO bridge.
    gTrimEnabled = (val != 0.0f);
    echo.print("# gTrimEnabled = ");
    echo.println(gTrimEnabled ? "TRUE (adapting)" : "FALSE (frozen at current value)");
  } else if (cmd == 'n') {
    // USB build's "k<value>". 'k' is the link keepalive here -- see header.
    gKAdapt = val;
    echo.print("# gKAdapt = "); echo.println(gKAdapt, 8);
    echo.println("#   (note's table: 20s->8.765e-6 30s->5.843e-6 "
    ...
    // a negative tau) instead of silently producing garbage. Same letter
    // as the USB build: 'f' is free across every corner stage and the
    // bridge relays it untouched.
    if (val > 0.0f) {
      gRateFilterHz = val;
      echo.print("# gRateFilterHz = "); echo.print(gRateFilterHz, 1);
      echo.println(" Hz");
    } else {
      echo.println("# REFUSED: f<Hz> needs a positive value (try 15-25).");
    }
  } else if (cmd == 'h') {
    gHalted = (val != 0.0f);
    if (gHalted && gArmed) {
    ...
    // Deliberately NOT gKAdapt, which the USB build binds here: a bare "k"
    // every 100 ms would silently zero the adaptation gain. It is 'n'.
  } else {
    echo.println("# unknown. use: a<0/1>  g<0..1>  c (re-resolve)  "
                  "z<0/1> (clear/seed trim)  y<0/1> (freeze/resume adapt)  "
                  "n<value> (gKAdapt)  f<Hz> (rate filter)  h<0/1>  "
...
void setup() {
  ...
  gBoot.println("\nstarted - CORNER STAGE 4: FULL LAW + AUTO TRIM + RATE FILTER (WiFi build, cube held by hand)");
  ...
  // 800 Hz, not 400 -- kept from the USB build, and it is NOT the folder
  // default: cube-bringup/Stage0c_IMUJitter.ino found that this loop runs at
  // 500 Hz (kPeriodMs=2) while the other sketches leave the IMU's own output
  // rate at 400 Hz, so the loop polls FASTER than the sensor produces new
  // samples and roughly 1 in 5 reads is a stale repeat. Stage0c tested and
  // validated 800 Hz specifically to fix this.
  ...
  gBoot.println("# (0.5 deg) -- get close to the resolved corner first, or send");
  gBoot.println("# z1 to fast-start (seeds trim so corrected phi reads ~0 now).");
  gBoot.println("# Trim then adapts AUTOMATICALLY while armed+quiescent -- watch");
  gBoot.println("# trim_x/y/z_deg converge in telemetry, no further z1 needed.");
  gBoot.println("# z0 clears trim. NOTE the two remapped letters on this build:");
  gBoot.println("#   y0/y1 freezes/resumes adaptation (USB build: x0/x1)");
  gBoot.println("#   n<value> sets the adaptation gain (USB build: k<value>),");
  gBoot.println("#   default k_a=1e-4 -- ~10 s to converge, not the old ~4 min.");
  gBoot.println("#   -- 'x' never reaches the Teensy, 'k' is the keepalive.");
  gBoot.print("# Rate low-pass at "); gBoot.print(gRateFilterHz, 1);
  gBoot.println(" Hz feeds the control law's om block only");
  gBoot.println("#   (om_x/y/z_filt_dps in telemetry, beside the RAW om_x/y/z_dps");
  gBoot.println("#   -- the gap between them is the 35 Hz mode being removed).");
  gBoot.println("#   f<Hz> sweeps 15-25 Hz live, same letter as USB. See");
  gBoot.println("#   hw-run-analysis.md fixes 4.1 and 4.5.");
  ...
  gBoot.println("#   d20 = 25 Hz readable -- use it to eyeball raw vs filtered),");
  gBoot.println("#   l<0/1> picks USB/WiFi telemetry, k is the link keepalive.");
  gBoot.println("#   AUTO-DISARMS if no line arrives on Serial1 for 300 ms, which");
  gBoot.println("#   also freezes trim adaptation but KEEPS the converged value.");
  gBoot.println("#   The rate filter keeps running regardless -- it is a");
  gBoot.println("#   measurement filter, not learned state.");
...
void loop() {
  ...
  // laptop would leave the law running. Dropping gArmed also stops
  // updateTrim() adapting, which is the wanted behavior: a link loss says
  // nothing about the equilibrium. The rate filter is deliberately NOT
  // touched here -- see the header.
  if (gLinkMode == LinkMode::WIFI &&
      millis() - gLastSerial1RxMillis > kLinkTimeoutMs) {
    if (gArmed) {
      gArmed = false;
      gBoot.println("# DISARMED: link lost (trim frozen at its current value)");
    }
    gLinkAlive = false;
  }
  ...
  // throttled. Neither the trim adaptation nor the rate filter is decimated
  // -- both step every cycle regardless of what d<N> is set to. That matters
  // more here than in ../Stage4_AutoTrim_WiFi/: a 20 Hz filter sampled at the
  // telemetry rate instead of the control rate would be a different filter.
...
// NOTES
// ----------------------------------------------------------------------------
// This file vs ../Stage4_AutoTrim_WiFi/: identical control law, identical
// corner candidate table, identical trim mechanism (guards, sign, clamp) --
// the ONLY behavioral differences are (1) w_filt replacing w_b in
// commandWheels()'s om block and in updateTrim()'s quiescence gate, per
// hw-run-analysis.md's fix 4.1, (2) gKAdapt's default raised from 2.922e-6 to
// 1e-4 per fix 4.5, and (3) the telemetry line growing from 26 to 29 fields.
// If something looks wrong here that isn't about the 35 Hz mode or
// trim-adaptation speed specifically, check whether the plain AutoTrim build
// has the same problem before assuming this file's changes caused it.
//
// This file vs ../../../../corner-bringup/Stage4_AutoTrim_RateFilter/:
// transport only. The link layer (LineReader, DualPrint, watchdog, k/l/d) is
// the same one every corner bring-up WiFi stage uses, and the two remapped
// command letters are documented in the header. 'f' did NOT have to move.
// Retune in the USB sketch, then re-copy.
//
// Next steps once this is trusted on real hardware (unchanged from the USB
// build's list):
//   - Port the SAME rate-filter + k_a replacement into a Stage 5 WiFi build.
//     The USB side already has Stage5_AutoTrim_RateFilter.ino; there is no
//     WiFi counterpart yet, and Stage5_Release_WiFi still carries the manual
//     gPhiOffset.
//   - Drop in the real Kp[3][9]/gB/ell/Sg/lambda/theta_eq for the remaining
//     SEVEN corners once available for the new plant (mass 1.633 kg + strut)
//     -- corner [-1,-1,-1]'s set is done (2026-08-19), see the TODO above
//     kCorners.
//   - EEPROM/LittleFS persistence -- save gTrim once converged, load it back
//     at boot instead of starting at zero every power-up.
//   - The 180-deg flip test (Automatic Trim.md S5): let trim converge, rotate
//     the cube 180 deg about the vertical on the SAME corner, let it converge
//     again. A COM offset reverses sign; an IMU mounting misalignment does
//     not -- this is the only way to tell the two apart.
//
