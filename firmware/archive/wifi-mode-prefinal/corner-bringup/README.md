...
| 4-FO | [`Stage4_FixedOffset/`](../../wireless-console-final/WIFIMODE/corner-bringup/Stage4_FixedOffset) | 26 | 4-AT with trim **hardcoded** to its converged value, and **no arm gate** |
| 4-FOF | [`Stage4_FixedOffset_RateFilter/`](../../../corner-bringup/Stage4_FixedOffset_RateFilter) | 29 | 4-FO plus 4-ATF's rate low-pass |
| 4-FOK | [`Stage4_FixedOffset_Kalman/`](../../../corner-bringup/Stage4_FixedOffset_Kalman) | 32 | 4-FO with the low-pass replaced by a **mode-augmented Kalman filter** — *experimental* |
...
`terminal_wifi.py` tees everything received to `session_<tag>_<stamp>.log` in [`../../telemetry/serial/`](../../../../README.md) (`--no-log` to skip) — the SERIALMONITORMODE half of the recordings folder, kept apart from the live plotters' PLOTMODE csv in `telemetry/plot/`. Because the header row is in the stream, [`../../plot_session_csv.py`](../../../cubli-ui/tools/analysis/plot_session_csv.py) opens a **Stage 5** log directly — 21 columns, though it will label the last one `gain_scale` when it is really `trip_reason`. Run it with no arguments and press `f` to pick the log from a list.
