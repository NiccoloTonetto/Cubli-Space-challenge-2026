/*
  ...
  A superset of the UDP bridge in PreFINAL/WIFIMODE/xiao_teensy_bridge/.
  That sketch is LEFT IN PLACE and still works; this one keeps its entire UDP
  behaviour byte-for-byte (same port, same x0/x1 mode switch, same verbatim
  relay) and adds three things on top:
  ...
  ota_base.ino), which is 4x the chunk size, so even a worst-case erase
  ...
  web_index.h is GENERATED. After editing ../../../UI/web/index.html run:
      cd ../../../UI/tools && python embed_web.py
