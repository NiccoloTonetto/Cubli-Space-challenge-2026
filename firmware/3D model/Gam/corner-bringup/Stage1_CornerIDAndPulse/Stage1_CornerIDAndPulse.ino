...
// comment, and re-confirmed through all of edge-bringup): id 2 -> X,
// id 3 -> Y, id 1 -> Z. Corner balance needs all three wheels live at once,
// so unlike edge-bringup there is no kAxis selector here -- every stage in
// this folder always instantiates all three.
...
// hardware there (Stage1_WheelSignCheck.ino). Re-verify per wheel rather
// than assuming it still holds if the mount or wiring changes (Firmware
// Lessons S4) -- this stage's checklist is exactly that re-verification.
