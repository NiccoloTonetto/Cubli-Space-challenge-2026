...
// CORNER STAGE 4: FULL LAW + RATE FILTER + FIXED OFFSET (no arm gate)
//                                                              [WiFi build]
// ============================================================================
// WiFi variant of ../../../../corner-bringup/Stage4_FixedOffset_RateFilter/.
// The control path -- corner table, estimator, full law, friction
// feedforward, kPhiOffset, the removed arm gate, the loosened velocity cap,
// and the 20 Hz rate low-pass w_filt -- is copied VERBATIM and must not be
...
//
//   a<0/1>    arm / disarm (NO arm gate -- see the fixed-offset note below)
//   g<0..1>   gain scale
//   c         re-resolve corner
...
//
// Unlike ../Stage4_AutoTrim_RateFilter_WiFi/, NO letter had to move here.
// That build had to remap 'x' (trim freeze) and 'k' (gKAdapt) because 'x'
...
// This file retired the whole trim mechanism, so it has nothing bound to
// either letter. 'f' was free across every corner stage in both builds and
// the bridge relays it verbatim, so it never had to move in either.
//
...
//
// ----------------------------------------------------------------------------
// Copy of Stage4_AutoTrim_RateFilter.ino with the SAME TWO changes
// Stage4_FixedOffset.ino made to Stage4_AutoTrim.ino: the live-adapting
// gTrim is replaced by a compile-time kPhiOffset constant, and the arm
// gate is removed -- "a1" always arms. The rate filter itself (w_filt,
// f<Hz>) is UNCHANGED and stays live-settable; only the trim and arm-gate
// mechanisms are touched. Read Stage4_FixedOffset.ino's header for the
// full reasoning on both of those changes (this header only covers what's
// different about combining them with the rate filter) -- and
// Stage4_AutoTrim_RateFilter.ino's header for the rate filter itself if
// this is the first RateFilter variant you're looking at.
//
// ---------------------------- WHAT AND WHY (fixed offset) ------------------
// kPhiOffset below is the SAME value Stage4_FixedOffset.ino uses --
// perfect_equilibrium_2.log's converged trim (mean over 132.41s, std <
// 0.0012 deg/axis), derived on a run of the PLAIN AutoTrim file (no rate
// filter). Using it here assumes the true equilibrium offset doesn't
// depend on whether the rate filter is active -- a reasonable assumption
// (the filter changes what the CONTROL LAW sees, not the physical
// equilibrium the cube settles at), but not independently re-verified on
// a rate-filter run specifically. If this file's phi at rest looks
// different from Stage4_FixedOffset.ino's, that assumption is the first
// thing to check.
//
// ---------------------------- ARM GATE REMOVED ------------------------------
// Same tradeoff as Stage4_FixedOffset.ino, repeated because it matters:
// kArmGate is gone from the 'a' command, so "a1" arms from any starting
// tilt. The Kp control law is only validated for small angles (recovery
// envelope ~2.7-3.1 deg worst case per Cube-Performance-Envelope-Results
// .md); the ONLY remaining backstop is kMaxTilt (25 deg auto-disarm,
// still in commandWheels()) -- placing the cube near equilibrium before
// "a1" is now operator discipline, not code-enforced.
//
// ---------------------------- WHAT AND WHY ---------------------------------
...
// safety check (the isfinite() trip checks) unfiltered and un-delayed --
// filtering those would add lag to exactly the signals that need to react
// fastest. w_filt only replaces w_b in commandWheels()'s xVec om block
// (the K2 path hw-run-analysis.md identified), matching the note's own
// scope: "nothing else in the design changes."
//
...
//       rest, with no trim adaptation doing any work -- that's the point
//       of hardcoding a validated offset.
//   [ ] om_x/y/z_filt_dps should visibly track smoother than the raw
//       om_x/y/z_dps columns -- if they look nearly identical, check
//       gRateFilterHz (default 20 Hz, "f<Hz>" to sweep) hasn't been set
//       too high to matter.
//
// Both checklist items are much easier to eyeball slowly, and 250 Hz is
// unreadable for either: "d20" (25 Hz) is the useful rate for the raw-vs-
// filtered comparison, "d2" puts it back for capture. Changing it re-prints
// the header, so the log stays self-describing.
//
// Velocity cap loosened (kMaxOmega, not literally removed) same as
// Stage4_AutoTrim_RateFilter.ino -- see that file's header for the full
// reasoning. Trips here are still LOOSE (hand-held) -- Stage 5 tightens
// to the real DISARM/OMEGA_CAP policy from cubli_gains.h.
// ============================================================================
...
// This stage's line is 29 fields (~230 B) -- three more than the plain
// ../Stage4_FixedOffset/'s 26, for the filtered-rate columns: ~58 kB/s at
// 250 Hz, comfortable against the 1 Mbaud link, but the UDP hop is the real
// ceiling (~105 lines/s measured in link-bringup Stage 6).
...
// ADDED to the raw measurement below, same sign/convention gTrim used
// (verified on the bench, cos(trim,wheels)=-1.0000 -- see Stage4_AutoTrim
// .ino's header) -- this is that same verified value, just no longer
// recomputed live.
...
// always), kept so the line stays a shape the existing analysis tools already
// recognize. om_x/y/z_dps stay RAW so they can be compared column against
// column with the filtered three -- that difference is the 35 Hz mode being
// kept out of the K2 path, and it is the only direct evidence the filter is
// doing anything.
//
...
void printState(Stream& out, uint32_t t_ms, const float rho[3], const float rhoLp[3],
  ...
  // Same 5 columns Stage4_AutoTrim_RateFilter.ino's telemetry has, kept
  // for format compatibility with the existing analysis tools -- but
  // these are now CONSTANTS, not a live readout. trim_enabled=0 always,
  // signaling "fixed, not adapting" to anyone comparing logs across files.
...
// existed in Stage4_AutoTrim_RateFilter.ino to refuse "a1" unless already
// near the resolved corner's equilibrium; that check is gone from the
// 'a' command below, deliberately.
...
// SECTION 2b-2 (rate filter, w_filt/updateRateFilter()) is UNCHANGED and
// still runs every cycle -- only the trim mechanism is removed here.


// ----------------------------------------------------------------------------
// SECTION 2f: COMMANDS (from either Serial or Serial1)
// ----------------------------------------------------------------------------
// The USB build's a/g/c/f/h grammar, byte-identical -- no letter had to move
...
void handleCommandLine(const char* line, Stream& echo) {
  ...
  if (cmd == 'a') {
    ...
    echo.println("# unknown. use: a<0/1> (arm/disarm, no gate)  "
                  "g<0..1> (gain scale)  c (re-resolve)  f<Hz> (rate "
                  "filter corner freq)  h<0/1> (halt)  d<N> (telem decim)  "
...
void setup() {
  ...
  gBoot.println("\nstarted - CORNER STAGE 4: FULL LAW + RATE FILTER + FIXED OFFSET, NO ARM GATE (WiFi build, cube held by hand)");
  ...
  gBoot.println(" Hz feeds the control law (om_x/y/z_filt_dps in");
  gBoot.println("# telemetry) -- f<Hz> to sweep 15-25 Hz live, see hw-run-analysis.md.");
  ...
  gBoot.println("#   AUTO-DISARMS if no line arrives on Serial1 for 3 s. The rate");
  gBoot.println("#   filter keeps running regardless -- it is a measurement");
  gBoot.println("#   filter, not learned state. No command letter moved here.");
...
void loop() {
  ...
  // automatic protections left. The rate filter is deliberately NOT touched
  // here -- see the header.
  ...
  // throttled. The rate filter is NOT decimated -- it steps every cycle
  // regardless of what d<N> is set to. A 20 Hz filter sampled at the
...
// NOTES
// ----------------------------------------------------------------------------
// This file vs Stage4_AutoTrim_RateFilter.ino: identical control law,
// identical corner candidate table, identical rate filter -- the
// difference is entirely in how phi's offset is sourced (compile-time
// constant here vs live adaptation there) and that the arm gate is gone.
// This file vs Stage4_FixedOffset.ino: identical fixed-offset/no-arm-gate
// treatment, PLUS the rate filter. If something looks wrong here, check
// which of those two comparison files has the same problem before
// assuming this file's own combination caused it.
//
// This file vs ../../../../corner-bringup/Stage4_FixedOffset_RateFilter/:
...
// had to move -- 'f' was already free everywhere, and this build has nothing
// bound to the 'x'/'k' letters that forced the AutoTrim WiFi builds to remap.
...
//     see the TODO above kCorners.
//   - If kPhiOffset is trusted across multiple sessions/days, it's a
//     candidate for promoting into cubli_gains.h's per-corner table
//     directly rather than living only here and in Stage4_FixedOffset.ino.
//   - Re-derive kPhiOffset (re-run Stage4_AutoTrim.ino or Stage4_AutoTrim
//     _RateFilter.ino, let it converge, take the new mean) after ANY
//     mechanical change to this corner -- this file has no way to detect
//     that its offset has gone stale on its own.
// ============================================================================
