...
// (tilt / omega / nan), same shape as edge-bringup's Stage 5 and the
// panel's before it, printed once at the moment it fires -- worth the
// extra visibility here specifically, since this is the one stage where
// nobody's hand is on the cube to feel a problem before the telemetry
// shows it, and there are three wheels' worth of stored momentum instead
// of one.
//
// STAGE 5 PROCEDURE:
...
//
// Still using the bench power supply, not the flight battery, same as the
// rest of this bring-up so far -- COM is still nominally on the pivot-to-
// COM line the corner gains were designed around, but Sg/lambda (and
// therefore how well THIS Kp performs) are still whatever they are for
// the current, not final, mass distribution. If it's stable but sluggish,
// or the resting equilibrium is measurably off from cubli_gains.h's
// per-corner theta_eq comments, that's consistent with a plant mismatch,
// not necessarily a sign/wiring bug -- re-derive gains once the final mass
// is on, same workflow as everywhere else in this project (measure ->
// derive -> re-tune). No live phi-offset correction is wired into this
// stage (unlike edge-bringup's kPhiOffset) -- that was added there only
// after a SPECIFIC measured resting-point shift on the Y edge; add the
// vector equivalent here only if the same problem actually shows up on a
// corner, not preemptively.
// ============================================================================
...
void printState(uint32_t t_ms, const float rho[3], const float rhoLp[3],
  ...
  Serial.print('\t'); Serial.println(tripReason);
}
...
void handleSerialCommands() {
  ...
  if (cmd == 'a') {
    ...
    Serial.println("# unknown. use: a<0/1>  c (re-resolve)  r<val> (log marker)  h<0/1>");
  }
}
...
void setup() {
  Serial.begin(115200);
  while (!Serial) {}
  Serial.println("started - CORNER STAGE 5: RELEASE (stop/catch + e-stop, cube free)");
...
// are actually mounted (measure -> derive -> re-tune, same workflow as
// everywhere else in this project), then repeat this entire folder's
// Stage 1->5 sequence for the OTHER SEVEN corners before trusting any of
