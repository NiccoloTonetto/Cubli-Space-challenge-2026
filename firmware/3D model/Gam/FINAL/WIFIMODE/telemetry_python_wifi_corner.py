...
COMMANDS -- type into the terminal you launched this from, press Enter:
    a1 / a0     arm / disarm (a1 goes through the 0.5 deg gate)
    g<0..1>     gain scale
    c           re-resolve the corner candidate
    z1 / z0     tare / clear the phi offset
    p1 / p0     HALT / resume   <-- NOT 'h': see below
    t0 / t1     Teensy link mode (USB / WiFi)
    x0 / x1     XIAO's own mode (WIFI_TEST / TEENSY_BRIDGE)

'h' IS THE KEEPALIVE, NOT HALT. A background thread sends a bare "h"
every HEARTBEAT_INTERVAL_S -- the firmware auto-disarms if its WiFi link
goes quiet for kLinkTimeoutMs (300 ms). Halt is therefore bound to 'p'
in the WiFi build. The USB build in ../USBMODE/CornerBalance/ still uses
h1/h0 for halt; that is the one grammar difference between them.
...
def heartbeat_thread(sock, xiao_addr, stop_event):
    ...
    laptop address fresh during idle periods between commands. The firmware
    treats 'h' as a no-op precisely so this can run unmodified."""
    while not stop_event.is_set():
        sock.sendto(b"h\n", xiao_addr)
...
def main():
    ...
    # draw the gate itself, so "am I close enough to arm?" is answered by
